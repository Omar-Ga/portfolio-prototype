...
const GalleryView: React.FC<Props> = ({ projects, onProjectClick, isSidebarCollapsed = false }) => {
  ...
  const renderProject = (project: Project, index: number, isPortrait: boolean = false) => (
    <motion.div
      ...
      className={`group relative overflow-hidden rounded-2xl bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-white/5 cursor-pointer ${isPortrait ? 'aspect-[9/16]' : 'aspect-square'}`}
    >
  ...
  return (
    <div className="flex flex-col w-full">
      {widescreenProjects.length > 0 && (
        <div className={`grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 ${isSidebarCollapsed ? 'xl:grid-cols-4 2xl:grid-cols-5' : ''} py-6 md:py-12 mx-auto transition-all duration-500 ease-in-out ${isSidebarCollapsed ? 'gap-2 md:gap-3 px-2 md:px-4 max-w-[1800px] w-full' : 'gap-4 md:gap-6 px-4 md:px-6 max-w-7xl w-full'}`}>
          {widescreenProjects.map((project, index) => renderProject(project, index, false))}
        </div>
      )}

      {portraitProjects.length > 0 && (
        <div className={`grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 ${isSidebarCollapsed ? 'xl:grid-cols-7 2xl:grid-cols-8' : ''} pb-6 md:pb-12 pt-0 mx-auto transition-all duration-500 ease-in-out ${isSidebarCollapsed ? 'gap-2 md:gap-3 px-2 md:px-4 max-w-[1800px] w-full' : 'gap-4 md:gap-6 px-4 md:px-6 max-w-7xl w-full'}`}>
